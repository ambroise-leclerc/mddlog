...
const speclab::Register consoleSinkOptionsSurviveConcurrentAccess{
    "ConsoleSink's color/stderr options survive concurrent getter and setter access", "unit", [] {
        struct State {
            ...
            .Then("every one of the 800 unique ids was delivered exactly once",
                  [](State& s) {
                      ...
                      constexpr std::size_t expected =
                          static_cast<std::size_t>(kConcurrencyProducers) * kConcurrencyPerProducer;
                      checks.expect(records.size() == expected,
                                    std::format("expected {} records, got {}", expected, records.size()));
                      ...
                      checks.expect(allExactlyOnce,
                                    "every expected id appears in the delivered records exactly once");
                      checks.raise();
                  })
            .Execute();
    }};
