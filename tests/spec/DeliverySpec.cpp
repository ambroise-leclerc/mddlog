...
const speclab::Register syncDeliveryIsImmediate{
    "A synchronous logger delivers a record before log() returns", "unit", [] {
        struct State {
        ...
        return speclab::Test<State>("delivery-sync-immediate")
            ...
            .When("a message is logged", [](State& s) { s.logger.info("synchronous"); })
            .Then("it is already present in the sink, with no flush needed",
                  [](State& s) {
                      if (s.sink->size() != 1) {
                          throw speclab::core::AssertionFailure(
                              std::format("expected 1 record immediately after log(), got {}",
                                          s.sink->size()),
...
const speclab::Register asyncDeliversExactlyOnce{
    "An asynchronous logger delivers every accepted record exactly once", "unit", [] {
        struct State {
            ...
            .Then("exactly 200 distinct records were delivered, none lost or duplicated",
                  [](State& s) {
                      ...
                      checks.expect(records.size() == static_cast<std::size_t>(kAsyncOnceCount),
                                    std::format("expected {} records, got {}", kAsyncOnceCount,
                                                records.size()));
...
const speclab::Register perProducerOrderingPreserved{
    "Per-producer ordering is preserved under concurrent async producers", "unit", [] {
        struct State {
        ...
        return speclab::Test<State>("delivery-per-producer-ordering")
            ...
            .Then("within each producer's messages, the sequence numbers are strictly increasing",
                  [](State& s) {
                      speclab::core::Checks checks;
                      const auto records = s.sink->records();
                      checks.expect(records.size() ==
                                        static_cast<std::size_t>(kOrderingProducers * kOrderingPerProducer),
                                    std::format("expected {} records, got {}",
                                                kOrderingProducers * kOrderingPerProducer, records.size()));
                      ...
                      for (const auto& record : records) {
                          int producer = -1;
                          int index = -1;
                          if (std::sscanf(record.message.c_str(), "p%d-%d", &producer, &index) == 2 &&
                              producer >= 0 && producer < kOrderingProducers) {
                      ...
                      checks.expect(orderedWithinProducer,
                                    "every producer's messages arrived in the order it sent them");
                      for (int p = 0; p < kOrderingProducers; ++p) {
