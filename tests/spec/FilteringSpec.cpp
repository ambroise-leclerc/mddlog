...
const speclab::Register loggerMinLevelFilters{
    "The logger's minimum level filters records below the threshold", "unit", [] {
...
const speclab::Register sinkMinLevelFiltersIndependently{
    "A sink's minimum level filters independently of the logger's", "unit", [] {
...
const speclab::Register disablingLoggerDropsRecords{
    "Disabling the logger drops ordinary records", "unit", [] {
        return speclab::Test<SingleSinkState>("filtering-logger-disabled")
            ...
            .When("an INFO message is logged", [](SingleSinkState& s) { s.logger.info("dropped"); })
...
const speclab::Register reEnablingLoggerResumesDelivery{
    "Re-enabling the logger resumes delivery", "unit", [] {
        return speclab::Test<SingleSinkState>("filtering-logger-re-enabled")
            ...
            .When("an INFO message is logged after re-enabling",
                  [](SingleSinkState& s) { s.logger.info("delivered after re-enable"); })
...
const speclab::Register multipleSinksAllReceive{
    "Every attached sink receives a logged record", "unit", [] {
        return speclab::Test<TwoSinkState>("filtering-multiple-sinks")
            ...
            .When("one message is logged", [](TwoSinkState& s) { s.logger.info("broadcast"); })
...
const speclab::Register removeSinkRemovesOnlyNamed{
    "removeSink() removes only the named sink", "unit", [] {
        return speclab::Test<TwoSinkState>("filtering-remove-sink")
            ...
            .When("one message is logged", [](TwoSinkState& s) { s.logger.info("after removal"); })
...
const speclab::Register clearSinksRemovesEverySink{
    "clearSinks() removes every sink and further records are simply dropped", "unit", [] {
        return speclab::Test<TwoSinkState>("filtering-clear-sinks")
            ...
            .When("one message is logged", [](TwoSinkState& s) { s.logger.info("into the void"); })
