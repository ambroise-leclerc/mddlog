...
const speclab::Register basicFieldsPreserved{
    "A basic log() call preserves level, message, and category", "unit", [] {
        return speclab::Test<SingleSinkState>("fidelity-basic-fields")
            .Given("a sink attached to a logger", [](SingleSinkState& s) { s.logger.addSink(s.sink); })
            .When("a WARN message is logged with an explicit category",
                  [](SingleSinkState& s) { s.logger.warn("elevated temperature", "vitals"); })
...
const speclab::Register medicalFieldsPreserved{
    "logMedical() preserves user, session and device identifiers", "unit", [] {
        return speclab::Test<SingleSinkState>("fidelity-medical-fields")
            .Given("a sink attached to a logger", [](SingleSinkState& s) { s.logger.addSink(s.sink); })
            .When("a medical compliance record is logged",
                  [](SingleSinkState& s) {
                      s.logger.logMedical(LogLevel::INFO, "monitoring started", "patient_monitor",
                                          "user123", "session456", "device789");
                  })
...
const speclab::Register auditFieldsPreserved{
    "logAudit() preserves the event type, risk level and AUDIT severity", "unit", [] {
...
const speclab::Register threadIdentityPreserved{
    "The record's thread id identifies the producer that logged it", "unit", [] {
        return speclab::Test<SingleSinkState>("fidelity-thread-identity")
            .Given("a sink attached to a logger", [](SingleSinkState& s) { s.logger.addSink(s.sink); })
            .When("a message is logged from this thread",
                  [](SingleSinkState& s) { s.logger.info("who logged this?"); })
                  ...
                  [](SingleSinkState& s) {
                      ...
                      if (records.empty() || records.front().threadId != std::this_thread::get_id()) {
                          throw speclab::core::AssertionFailure(
                              "record thread id did not match the logging thread",
...
const speclab::Register sourceLocationPointsAtCallSite{
    "The captured source location points at the caller, not inside Logger.cppm", "unit", [] {
        struct State {
        ...
        return speclab::Test<State>("fidelity-source-location")
            .Given("a sink attached to a logger", [](State& s) { s.logger.addSink(s.sink); })
            ...
            .Then("the record's location names this file and that exact line",
                  [](State& s) {
                      ...
                      if (!records.empty()) {
                          ...
                          checks.expect(file.find("Logger.cppm") == std::string_view::npos,
                                        "location file is not inside Logger.cppm");
                          checks.expect(records.front().location.line() == s.expectedLine,
                                        std::format("location line is {}, expected {}",
                                                     records.front().location.line(), s.expectedLine));
                      }
                      checks.raise();
                  })
            .Execute();
