...
const speclab::Register throwingSinkDoesNotBlockOthers{
    "A throwing sink does not prevent delivery to the other sinks", "unit", [] {
        return speclab::Test<SyncState>("sink-failure-does-not-block-others")
            ...
            .When("a message is logged", [](SyncState& s) { s.logger.info("survives the throw"); })
...
const speclab::Register writeFailureIsRecordedExplicitly{
    "A caught sink write failure is recorded explicitly in that sink's own statistics", "unit", [] {
        return speclab::Test<SyncState>("sink-failure-recorded-explicitly")
            .Given("a throwing sink attached to a logger", [](SyncState& s) { s.logger.addSink(s.throwing); })
            .When("two messages are logged", [](SyncState& s) {
                s.logger.info("first");
...
const speclab::Register throwingSinkDoesNotKillAsyncWorker{
    "A throwing sink does not crash the asynchronous worker thread", "unit", [] {
        return speclab::Test<AsyncState>("sink-failure-async-worker-survives")
            ...
            .Then("the worker kept running: every message still reached the recording sink",
                  [](AsyncState& s) {
                      speclab::core::Checks checks;
                      checks.expect(s.throwing->attempts() == 20,
                                    std::format("throwing sink saw 20 attempts, got {}",
                                                s.throwing->attempts()));
                      checks.expect(s.recording->size() == 20,
                                    std::format("recording sink saw 20 deliveries, got {}",
                                                s.recording->size()));
                      checks.raise();
                  })
            .Execute();
